...
router.get('/', validatesession, async (req, res) => {
    try {
        ...
        const query = req.user.rol === 'tecnico'
            ? `${queryBase} WHERE r.id_tecnico = ${req.user.id_usuario} ORDER BY r.fecha_ingreso DESC`
            : `${queryBase} ORDER BY r.fecha_ingreso DESC`;

        const [repairs] = await con.query(query);
